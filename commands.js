...
commands.saveOrLoad = function(info){
	...
	if(Sburb.isStateInStorage(false,local)){
		actions.push(new Sburb.Action("load","false, "+local,"Cargar "+Sburb.getStateDescription(false)+"."));
	}
	if(Sburb.isStateInStorage(true,local)){
		actions.push(new Sburb.Action("load","true, "+local,"Cargar "+Sburb.getStateDescription(true)+"."));
	}
	if(Sburb.tests.storage) {
	    actions.push(new Sburb.Action("save","false,"+local,"Guardar."));
    }
	actions.push(new Sburb.Action("cancel",null,"Cancelar."));
	Sburb.chooser.choices = actions;
	Sburb.chooser.beginChoosing(Sburb.Stage.x+20,Sburb.Stage.y+50);
}
